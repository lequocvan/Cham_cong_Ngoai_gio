...
def get_user_v1_permission(user):
    ...
    # ID Phòng Tổng hợp = 2 (hoặc Admin)
    is_tong_hop = (is_admin or user_ma_pb in [2])
...
def bang_luong_v1():
    ...
    # Quyền Admin hoặc phòng Tổng hợp (ID phòng ban = 2)
    user_role = getattr(current_user, 'role', '')
    is_admin_or_th = (current_user.is_admin == 1 or user_role == 'ADMIN' or user_ma_pb in [2])
    is_kiem_soat = (user_role == 'KIEM_SOAT')
    ...
    # Lọc danh sách phòng ban ban đầu theo đơn vị được chọn
    if selected_don_vi:
    ...
    if is_admin_or_th:
        # Admin / Phòng Tổng hợp: Được xem toàn bộ và lọc theo giao diện
        if selected_don_vi:
            sql_text += " AND nv.ma_hieu_2 = :dv"
        ...
        # KIỂM SOÁT: Chỉ xem bản thân VÀ tất cả nhân viên thuộc cùng phòng ban (và cùng đơn vị)
        sql_text += " AND (nv.ma_phong_ban = :user_ma_pb OR nv.ma_nhan_vien = :current_ma_nv)"
        params["user_ma_pb"] = user_ma_pb
        params["current_ma_nv"] = current_user.ma_nhan_vien

        if user_dv:
            sql_text += " AND nv.ma_hieu_2 = :user_dv"
            params["user_dv"] = user_dv

        # Cho phép lọc thêm mã NV trong phạm vi phòng ban đó
        if ma_nv_filter:
            sql_text += " AND nv.ma_nhan_vien LIKE :ma"
            params["ma"] = f"%{ma_nv_filter}%"

    else:
        # User thường: Chỉ xem duy nhất bản thân
        sql_text += " AND nv.ma_nhan_vien = :current_ma_nv"
        params["current_ma_nv"] = current_user.ma_nhan_vien

    sql_text += " ORDER BY pb.ten_phong_ban, nv.ma_nhan_vien"
...
# Route cập nhật thông tin khai báo V1 (Áp dụng từ ngày, hệ số, đính kèm, ghi chú)
@app.route('/update-v1-item', methods=['POST'])
@login_required
def update_v1_item():
    # 1. Kiểm tra quyền người dùng đang đăng nhập
    user_info = ThongTinNguoiLaoDong.query.filter_by(ma_nhan_vien=current_user.ma_nhan_vien).first()
    user_ma_pb = user_info.ma_phong_ban if user_info else None
    ...
    # 3. PHÂN QUYỀN CẬP NHẬT: Kiểm tra xem user có phải chính chủ không (nếu không phải Admin/TH)
    if not is_admin_or_th and v1_row.ma_nhan_vien != current_user.ma_nhan_vien:
    ...
    if v1_row.approved_by and v1_row.approved_by not in ['None', '']:
    ...
    # 1. Thu thập giá trị 12 tháng từ FormData
    months_params = {}
    set_months_sql = []
    for m in range(1, 13):
    ...
    # Ghép đoạn SQL động bao gồm cả t1 -> t12
    update_sql = f"""
        UPDATE thong_tin_v1
    ...
    # Khởi tạo Dictionary tham số chuẩn
    params = {
        'he_so_v1': he_so_v1,
    ...
    # Gộp toàn bộ t1..t12 vào dictionary tham số thực thi
...
# Route xử lý phê duyệt Hệ số lương V1 (Dành cho Admin/KIEM_SOAT)
@app.route('/approve-v1', methods=['POST'])
@login_required
def approve_v1():
    # 1. Kiểm tra vai trò cơ bản
    user_role = getattr(current_user, 'role', '')
    is_admin = (getattr(current_user, 'is_admin', 0) == 1 or user_role == 'ADMIN')

    if not is_admin and user_role != 'KIEM_SOAT':
    ...
    updates = data.get('updates', []) # Danh sách hệ số lương v1 mới

    # 2. Truy vấn thông tin phòng ban & đơn vị chuẩn từ hồ sơ lao động
    user_info = ThongTinNguoiLaoDong.query.filter_by(ma_nhan_vien=current_user.ma_nhan_vien).first()
    user_pb = user_info.ma_phong_ban if user_info else None
    user_dv = user_info.ma_hieu_2 if user_info else None

    # Kiểm tra bắt buộc đối với KIEM_SOAT
    if user_role == 'KIEM_SOAT' and not is_admin:
        if user_pb is None:
    ...
    try:
        ...
        if updates:
            ...
            # Ép quyền Phòng ban & Đơn vị trực tiếp cho KIEM_SOAT trong SQL
            if user_role == 'KIEM_SOAT' and not is_admin:
            ...
            for item in updates:
                ...
                if user_role == 'KIEM_SOAT' and not is_admin:
                    params_save['user_pb'] = user_pb
                    if user_dv:
                        params_save['user_dv'] = user_dv
        ...
        # Áp dụng bộ lọc phân quyền chuẩn xác
        if is_admin:
            # ADMIN: Lọc theo filter truyền lên từ client
            if selected_don_vi and str(selected_don_vi).strip():
            ...
            # KIEM_SOAT: Ép cứng theo Phòng ban & Đơn vị của tài khoản đăng nhập
            approve_query += " AND nv.ma_phong_ban = :user_pb"
            params_approve["user_pb"] = user_pb
            if user_dv:
                approve_query += " AND nv.ma_hieu_2 = :user_dv"
                params_approve["user_dv"] = user_dv

        # Cho phép lọc thêm Mã NV (nếu tìm kiếm cụ thể trên màn hình)
        ...
        if row_count == 0:
            return jsonify({
                'success': True, 
                'message': 'Không tìm thấy dòng dữ liệu mới nào cần phê duyệt (hoặc ngoài phạm vi phòng ban được cấp quyền).'
            })

        log_event("Phê duyệt V1", f"User {current_user.ma_nhan_vien} ({user_role}) đã duyệt {row_count} bản ghi năm {year}")
...
def unapprove_v1():
    # 1. Kiểm tra vai trò
    user_role = getattr(current_user, 'role', '')
    is_admin = (current_user.is_admin == 1 or user_role == 'ADMIN')

    if user_role not in ['ADMIN', 'KIEM_SOAT'] and not is_admin:
        return jsonify({'success': False, 'message': 'Bạn không có quyền gỡ phê duyệt!'}), 403

    data = request.get_json() or {}
    year = data.get('year')
    ma_nv = data.get('ma_nv')  # Mã nhân viên cần gỡ duyệt

    if not year or not ma_nv:
        return jsonify({'success': False, 'message': 'Thiếu thông tin năm hoặc mã nhân viên!'}), 400

    # 2. Không cho phép KIEM_SOAT tự gỡ phê duyệt cho chính mình (chỉ ADMIN mới được)
    if not is_admin:
        if str(ma_nv).strip() == str(current_user.ma_nhan_vien).strip():
            return jsonify({
                'success': False, 
                'message': 'Bạn không được phép tự gỡ phê duyệt cho chính mình! Báo tin cho Admin mở khóa.'
            }), 403

    try:
        # 3. TRUY VẤN CÓ JOIN ĐỂ KIỂM TRA PHÂN QUYỀN CẤP PHÒNG BAN
        unapprove_query = """
            UPDATE thong_tin_v1 v1
            JOIN thong_tin_nguoi_lao_dong nv ON v1.ma_nhan_vien = nv.ma_nhan_vien
        ...
        # Nếu là KIEM_SOAT, lấy thông tin phòng ban từ thong_tin_nguoi_lao_dong, gỡ duyệt phải thuộc CÙNG PHÒNG BAN (và đơn vị)
        if user_role == 'KIEM_SOAT' and not is_admin:
            user_info = ThongTinNguoiLaoDong.query.filter_by(ma_nhan_vien=current_user.ma_nhan_vien).first()
            
            if not user_info or user_info.ma_phong_ban is None:
                return jsonify({'success': False, 'message': 'Tài khoản Kiểm soát chưa được gán phòng ban trong hồ sơ!'}), 403
            
            user_pb = user_info.ma_phong_ban
            user_dv = user_info.ma_hieu_2

            # Bắt buộc nhân viên được gỡ duyệt phải thuộc CÙNG PHÒNG BAN
            unapprove_query += " AND nv.ma_phong_ban = :pb"
            params["pb"] = user_pb

            # Kết hợp thêm Đơn vị để đảm bảo an toàn tuyệt đối
            if user_dv:
        ...
        if result.rowcount == 0:
            return jsonify({
                'success': False, 
                'message': 'Không thể gỡ duyệt (Bản ghi không tồn tại hoặc không thuộc phòng ban bạn quản lý)!'
            }), 403

        log_event(
            "Gỡ phê duyệt V1", 
            f"User {current_user.ma_nhan_vien} ({current_user.role}) gỡ duyệt cho NV {ma_nv} năm {year}"
        )
...
def unapprove_v1_batch():
    try:
        ...
        # Xây dựng query động
        query = """
            UPDATE bang_luong_v1 
            SET approved_by = NULL, approved_at = NULL 
            WHERE year = %s AND approved_by IS NOT NULL
        """
        params = [year]

        if ma_hieu_2:
            query += " AND ma_hieu_2 = %s"
            params.append(ma_hieu_2)
            
        if phong_ban:
            query += " AND phong_ban = %s"
            params.append(phong_ban)

        # Execute trực tiếp trên MySQL - triệu dòng chỉ mất vài trăm ms

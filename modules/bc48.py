# Import db từ extensions, KHÔNG import từ app; file extensions.py (cùng cấp với app.py)
...
# --- HÀM LẤY ENGINE CHO BC48 ---
def get_bc48_engine(target_db=None):
    """
    Trả về SQLAlchemy Engine cho bind 'db_bc48'.
    Mặc định lấy từ instance `db` đã được init_app.
    """
    active_db = target_db or db
    return active_db.get_engine(bind='db_bc48')
                ...
                if file_hinh_thuc == "GBS":
                    ...
                    if list_ma_gd:
                        ...
                        for chunk_idx in range(0, len(list_ma_gd_unique), chunk_size):
                            ...
                            ##    "t_like": f"{t_clean}%",  # Dùng LIKE '20260615%' tận dụng index thoidiem thay vì LEFT()
                            ##    "m_list": list(unique_macns),
                            ##    "magd_list": sub_magd_list
                            ##})
                            ##total_deleted += result.rowcount
                        ##print(f">>> [DEBUG GLA] File {filename}: Đã dọn sạch {total_deleted} dòng lỗi cũ bằng Index.", flush=True)
...
####################################################################################
def get_file_logs_query(start_date, end_date, loai_bc, hinh_thuc=None, file_name=None, exact_match=True, page=1, per_page=50):
    # Tính toán offset
    ...
    # Lọc hình thức
    if hinh_thuc and hinh_thuc.strip():
        query += " AND l.hinh_thuc = :hinh_thuc"
        params['hinh_thuc'] = hinh_thuc.strip()

    # TỐI ƯU HIỆU NĂNG TÌM KIẾM TÊN FILE
    if file_name and file_name.strip():
        f_name = file_name.strip()
        if exact_match:
            # Lọc chính xác = Tận dụng Index 100% (Tốc độ vài milliseconds)
            query += " AND l.file_name = :file_name"
            params['file_name'] = f_name
        else:
            # Tìm chứa chuỗi (%f_name%)
            query += " AND l.file_name LIKE :file_name"
            params['file_name'] = f"%{f_name}%"
...
def get_file_logs_stats(start_date, end_date, loai_bc, hinh_thuc=None, file_name=None, exact_match=True):
    ...
    query = """
        SELECT COUNT(DISTINCT file_name) as unique_files, 
               ...
               COALESCE(SUM(so_luong), 0) as total_so_luong
...
def get_file_logs_options():
    """Lấy danh sách duy nhất của loai_bc và hinh_thuc để làm dropdown filter"""
    query_loai_bc = """
    ...
    """
    query_hinh_thuc = """
        SELECT DISTINCT hinh_thuc 
        FROM log_file_imports 
        WHERE hinh_thuc IS NOT NULL AND hinh_thuc != '' 
        ORDER BY hinh_thuc ASC
    """
    return query_loai_bc, query_hinh_thuc
            ...
            with engine.begin() as transaction_conn:
                for ma_don_vi in df_all['macn'].unique():
                    ...
                    if loai_mat_khau == 'random':
                        ...
                        secret_password = tao_mat_khau_co_dinh(ma_cn_chuandanh, loai_bc_thuc_te)
...
def send_via_opened_server(server, from_email, to_email, cc_email, ma_cn, file_path, loai_bc):
    ...
    try:
        ...
        msg['Subject'] = f"[{loai_bc}] DỮ LIỆU LỖI BÁO CÁO - ĐƠN VỊ {ma_cn} - {datetime.now().strftime('%d/%m/%Y')}"
        
        body = (f"Kính gửi Đơn vị {ma_cn},\n\n"
                f"Trung tâm PCRT gửi danh sách dữ liệu báo cáo lỗi nghiệp vụ {loai_bc} do Ngân hàng Nhà nước phản hồi.\n"
                f"Chi tiết vui lòng giải nén tệp tin đính kèm bằng mật khẩu cố định quy định của đơn vị.\n\n"
                f"Trân trọng.")
...
################################################################################################################
def run_sp_tong_hop_sodong_bang_error(db):
    try:
        # 1. Khai báo engine
        engine = get_bc48_engine(db)

        # 2. Thực thi Stored Procedure qua Connection của SQLAlchemy
        with engine.begin() as connection:
            connection.execute(text("CALL sp_tong_hop_sodong_bang_error();"))
        
        return True, "Chạy tổng hợp dữ liệu thành công!"
    except Exception as e:
        if 'db' in locals() and hasattr(db, 'rollback'):
            db.rollback()
        return False, f"Lỗi khi chạy Stored Procedure: {str(e)}"
